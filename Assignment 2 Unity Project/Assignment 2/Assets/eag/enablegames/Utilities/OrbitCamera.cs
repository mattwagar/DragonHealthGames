...
public class OrbitCamera : MonoBehaviour {
	...
	//global transform to lookat assigned originally to Target's transform
	...
	public void LateUpdate () {
		if (target) {
			//Vector3 savePosition = lookatTarget.position;
...
//			distance += Input.GetAxis("Fire2") *zoomSpd* 0.02f;
			...
			if (Input.GetKey(KeyCode.K))
				distance += zoomSpd* 0.02f;
//			distance += Input.GetKeyDown(KeyCode.J) *zoomSpd* 0.02f;

			Quaternion rotation = Quaternion.Euler(y, x, 0.0f);
			Vector3 position = rotation * new Vector3(0.0f, 0.0f, -distance) + lookatTarget.position;

			transform.rotation = rotation;
			transform.position = position;
			//lookatTarget.position = savePosition;
		}
	}
